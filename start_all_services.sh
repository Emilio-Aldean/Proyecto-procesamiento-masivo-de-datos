...
# ARREGLO AUTOMÁTICO: Verificar conectividad a HDFS
log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1"
}

log "Verificando conectividad a HDFS..."
if ! nc -z namenode 9000 2>/dev/null; then
    log "ERROR: No se puede conectar a namenode:9000"
    log "Esto indica que el contenedor no está en la red docker-hadoop_default"
    log "SOLUCIÓN: Ejecutar manualmente: docker network connect docker-hadoop_default <container_name>"
    exit 1
else
    log "✅ Conectividad a HDFS verificada correctamente"
fi
